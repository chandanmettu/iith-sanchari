...
- README rewritten as a handover document. It covers the live URL, the real repo slug (`chandanmettu/iith-sanchari`; the local folder was renamed `Sanchari` the same day), the SSH alias, the deploy path, the reading order, the release checklist and the rules.
