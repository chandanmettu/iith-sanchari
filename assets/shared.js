...
window.Sanchari = (() => {
  ...
  function departures(cfg, direction, count=6, now=Date.now(), holidays=[]) {
    ...
    for(let off=0;off<15&&out.length<count;off++){
      ...
      if(holidays.includes(key)||(cfg.weekdays&&['Sat','Sun'].includes(day)))continue;
      for(const hm of cfg[direction]){const ts=new Date(key+'T'+hm+':00+05:30').getTime();if(ts>now){out.push(ts);if(out.length===count)break;}}
    }return out;
  }
  ...
  return {date,dayKey,clock,when,escape,departures,api,saved,save,recovery};
})();
